...
import { MCPConfig, MCPServiceConfig, AuthConfig } from '../../types';
...
export class GatewayConfigManager {
  ...
  static loadFromEnvironment(): MCPConfig {
    const config: MCPConfig = {
      ...
      retryPolicy: {
        ...
        backoffStrategy: (process.env.MCP_RETRY_STRATEGY as 'linear' | 'exponential') || 'exponential',
   ...
   */
  private static loadServiceConfig(serviceName: string): MCPServiceConfig | null {
    ...
    const authType = process.env[`${prefix}_AUTH_TYPE`] as 'bearer' | 'api-key' | 'oauth2';
    if (!authType) {
   ...
   */
  private static loadAuthCredentials(prefix: string, authType: string): Record<string, string> {
   ...
   */
  private static validateServiceConfig(serviceName: string, config: MCPServiceConfig): string[] {
    const errors: string[] = [];
