...
export class AuthManager {
   ...
   */
  private getValidToken(serviceId: string, authConfig: AuthConfig): string | null {
   ...
   */
  async refreshOAuth2Token(serviceId: string, authConfig: AuthConfig): Promise<TokenRefreshResult> {
   ...
   */
  getTokenInfo(serviceId: string): { hasToken: boolean; expiresAt?: Date } {
    const cached = this.tokenCache.get(serviceId);
    return {
      hasToken: !!cached,
