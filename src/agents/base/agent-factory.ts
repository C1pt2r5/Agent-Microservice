...
import { MCPClientFactory } from '../../integration/mcp/mcp-client-factory';
import { MCPClientImpl } from '../../integration/mcp/mcp-client';
import { A2AClientImpl } from '../../integration/a2a/a2a-client';
import { GeminiClient } from '../../integration/gemini/gemini-client';
...
export class AgentFactory {
  ...
  static async createAgent(
    ...
    switch (config.type) {
      case 'chatbot':
        ...
        agent = new ConcreteBaseAgent(config, resolvedDependencies);
        break;
   ...
   */
  static async createDevelopmentAgent(
    agentId: string,
    agentType: 'chatbot' | 'fraud-detection' | 'recommendation'
  ): Promise<BaseAgent> {
   ...
   */
  static async createProductionAgent(
    agentId: string,
    agentType: 'chatbot' | 'fraud-detection' | 'recommendation'
  ): Promise<BaseAgent> {
    const config: AgentConfig = {
  ...
  static validateConfig(config: AgentConfig): string[] {
    ...
    if (!config.environment) errors.push('Agent environment is required');
    ...
    if (config.mcpEndpoint) {
      ...
      if (config.mcpEndpoint.timeout <= 0) errors.push('MCP timeout must be positive');
      if (config.mcpEndpoint.retryAttempts < 0) errors.push('MCP retry attempts must be non-negative');
    }

    // A2A endpoint validation
    if (config.a2aEndpoint) {
      if (!config.a2aEndpoint.url) errors.push('A2A endpoint URL is required');
      if (config.a2aEndpoint.timeout <= 0) errors.push('A2A timeout must be positive');
      if (config.a2aEndpoint.retryAttempts < 0) errors.push('A2A retry attempts must be non-negative');
    }
    ...
    if (config.geminiConfig) {
      ...
      if (!config.geminiConfig.endpoint) errors.push('Gemini endpoint is required');
      if (config.geminiConfig.maxTokens <= 0) errors.push('Gemini max tokens must be positive');
      ...
      if (config.geminiConfig.rateLimitPerMinute <= 0) errors.push('Gemini rate limit must be positive');
    }

    // Capabilities validation
    if (config.capabilities) {
      config.capabilities.forEach((capability, index) => {
        if (!capability.name) errors.push(`Capability ${index}: name is required`);
        if (!capability.description) errors.push(`Capability ${index}: description is required`);
      });
    }
  ...
  private static async resolveDependencies(
    config: AgentConfig, 
    providedDependencies?: AgentDependencies
    ...
    if (providedDependencies?.mcpClient) {
      ...
      dependencies.mcpClient = MCPClientFactory.createDevelopmentClient(config.mcpEndpoint.url) as MCPClientImpl;
    }
    ...
    if (providedDependencies?.a2aClient) {
      ...
      dependencies.a2aClient = new A2AClientImpl(a2aConfig);
    }
    ...
    if (providedDependencies?.geminiClient) {
      ...
      dependencies.geminiClient = new EnhancedGeminiClient(config.geminiConfig);
    }
   ...
   */
  private static getDefaultCapabilities(agentType: string) {
    switch (agentType) {
      case 'chatbot':
        return [
          {
   ...
   */
  static async createAgentCluster(
    configs: AgentConfig[],
    options: AgentFactoryOptions = {}
  ): Promise<BaseAgent[]> {
   ...
   */
  static async shutdownAgentCluster(agents: BaseAgent[]): Promise<void> {
    const shutdownPromises = agents.map(agent => 
      agent.shutdown().catch(error => 
        console.error(`Error shutting down agent ${agent.getConfig().id}:`, error)
      )
    );
