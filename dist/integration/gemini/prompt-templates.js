...
class PromptTemplateManager {
     ...
     */
    static validateVariables(templateId, variables) {
        const template = this.getTemplate(templateId);
        if (!template) {
            throw new Error(`Template not found: ${templateId}`);
        }
     ...
     */
    static createCustomTemplate(template) {
     ...
     */
    static extractVariables(templateString) {
     ...
     */
    static getTemplateSuggestions(useCase, category) {
        const templates = category ? this.getTemplatesByCategory(category) : this.getAllTemplates();
        return templates.filter(template => template.name.toLowerCase().includes(useCase.toLowerCase()) ||
