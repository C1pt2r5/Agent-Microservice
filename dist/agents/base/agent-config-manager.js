...
class AgentConfigManager {
     ...
     */
    static loadFromEnvironment(agentType, agentId) {
     ...
     */
    static createDevelopmentConfig(agentType, agentId, overrides = {}) {
        const template = this.CONFIG_TEMPLATES[agentType];
        if (!template) {
            throw new Error(`Unknown agent type: ${agentType}`);
        }
     ...
     */
    static createProductionConfig(agentType, agentId, overrides = {}) {
        const template = this.CONFIG_TEMPLATES[agentType];
        if (!template) {
            throw new Error(`Unknown agent type: ${agentType}`);
        }
     ...
     */
    static generateEnvTemplate(agentType) {
        const template = this.CONFIG_TEMPLATES[agentType];
        if (!template) {
            throw new Error(`Unknown agent type: ${agentType}`);
        }
...
AgentConfigManager.CONFIG_TEMPLATES = {
    chatbot: {
        name: 'Chatbot Agent',
...
//# sourceMappingURL=agent-config-manager.js.map